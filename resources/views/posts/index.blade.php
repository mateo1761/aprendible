    ...
    <main class="mx-auto grid w-full gap-4 px-4 max-w-7xl sm:grid-cols-2 md:grid-cols-3">
        @foreach($posts as $post)
            ...
            <div class="max-w-3xl px-4 py-2 space-y-4 bg-white rounded shadow dark:bg-slate-800">
                <h2>
                    <a class="text-xl text-slate-600 dark:text-slate-300 hover:underline" href="{{ route('posts.show', $post) }}">
                        {{ $post->title }}
                    </a>
                </h2>
                ...
                @auth
                    <div class="flex justify-between">
                        ...
                        <form action="{{ route('posts.destroy', $post)}}" method="post">
                            @csrf
                            @method('DELETE')
                            <button class="inline-flex items-center text-xs font-semibold tracking-widest text-center text-red-500 uppercase transition duration-150 ease-in-out dark:text-red-500/80 hover:text-red-600 focus:outline-none" type="submit" name="delete">Delete</button>
                        </form>
                    </div>
                @endauth
